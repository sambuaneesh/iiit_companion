...
def run_microservice(module_name, service_name, port):
    try:
        ...
        logger.info(f"Started {service_name} service on port {port}. Process ID: {current_process.pid}")

        # Log open ports for this process
        connections = current_process.connections()
        for conn in connections:
            logger.info(f"{service_name} service is listening on {conn.laddr.ip}:{conn.laddr.port}")

    except ImportError as e:
        logger.error(f"Error importing module for {service_name}: {str(e)}", exc_info=True)
    except AttributeError as e:
        logger.error(f"Error finding start function for {service_name}: {str(e)}", exc_info=True)
            ...
            for service in os.listdir(category_path):
                if service.endswith('.py') and not service.startswith('__'):
                    ...
                    if port_key in config.microservice_ports:
                        ...
                        logger.info(f"Starting thread for {service_name} service on port {port}")
                        thread = threading.Thread(target=run_microservice, args=(module_name, service_name, port))
                        thread.start()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for process in psutil.process_iter(['pid', 'name', 'cmdline']):
            if 'python' in process.info['name'].lower() and 'run_microservices.py' in ' '.join(process.info['cmdline']):
                logger.info(f"PID: {process.info['pid']}, Command: {' '.join(process.info['cmdline'])}")
